...
// ─────────────────────────────────────────────
// ERRORS PAGE (NOTEBOOK)
// ─────────────────────────────────────────────
function ErrorsPage({notebooks,setNotebooks}){
  const [active,setActive]=useState("Clínica Médica");
  const [saved,setSaved]=useState(false);
  const timer=useRef(null);
  function onChange(html){
    setNotebooks(prev=>{
      const next={...prev,[active]:html};
  ...
  function countMarks(html){ return (html?.match(/🔖/g)||[]).length; }
  function hasContent(html){ return (html||"").replace(/<[^>]*>/g,"").trim().length>0; }

  function exportPDF(){
    const html=notebooks[active]||"<p>Caderno vazio.</p>";
    const w=window.open("","_blank");
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Caderno — ${active}</title>
    <style>body{font-family:Georgia,serif;max-width:820px;margin:40px auto;padding:0 40px;color:#1a1a2e;line-height:1.8}
    ...
    ul,ol{padding-left:24px}li{margin:4px 0}hr{border:none;border-top:2px solid #e0e0f0;margin:20px 0}
    mark{padding:2px 5px;border-radius:3px}img{max-width:100%;border-radius:8px;margin:10px 0}
    .footer{margin-top:60px;font-size:11px;color:#aaa;border-top:1px solid #eee;padding-top:10px}
    </style></head><body>
    <h1>${active}</h1><p style="color:#aaa;font-size:13px">Caderno de Erros · ${new Date().toLocaleDateString("pt-BR")}</p>
    ${html}<div class="footer">Residência Médica · ${active}</div></body></html>`);
    w.document.close(); setTimeout(()=>{w.focus();w.print();},400);
  }
  function exportAll(){
    const parts=GRANDES_AREAS.map(a=>{
      const h=notebooks[a]||"";
      if(!h.replace(/<[^>]*>/g,"").trim()) return "";
      return `<div style="page-break-before:always"><h1 style="color:#6366f1;border-bottom:2px solid #6366f1;padding-bottom:10px">${a}</h1>${h}</div>`;
    }).filter(Boolean);
    if(!parts.length) return;
    const w=window.open("","_blank");
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Caderno Completo</title>
    <style>body{font-family:Georgia,serif;max-width:820px;margin:40px auto;padding:0 40px;color:#1a1a2e;line-height:1.8}
    ...
    <div class="cover"><h1 style="font-size:32px;color:#6366f1">Caderno de Erros</h1>
    <p style="color:#888">Residência Médica · ${new Date().toLocaleDateString("pt-BR")}</p></div>
    ${parts.join("")}</body></html>`);
    w.document.close(); setTimeout(()=>{w.focus();w.print();},400);
  }

  return(
    <div style={{display:"flex",height:"calc(100vh - 52px)",gap:0,margin:"-26px -28px",overflow:"hidden"}}>
      {/* Sidebar */}
      <div style={{width:200,flexShrink:0,borderRight:"1.5px solid #ebebf3",padding:"16px 10px",display:"flex",flexDirection:"column",gap:3,background:"#fafafa",overflowY:"auto"}}>
        <div style={{fontSize:11,fontWeight:700,color:"#aaa",textTransform:"uppercase",letterSpacing:1,padding:"0 6px 10px"}}>Caderno de Erros</div>
        {GRANDES_AREAS.map(area=>{
          const marks=countMarks(notebooks[area]);
          const has=hasContent(notebooks[area]);
          return(
            <div key={area} onClick={()=>setActive(area)} style={{padding:"9px 10px",borderRadius:9,cursor:"pointer",background:active===area?AREA_BG[area]:"transparent",border:active===area?`1.5px solid ${AREA_COLOR[area]}30`:"1.5px solid transparent",transition:"all .15s"}}>
              <div style={{display:"flex",gap:7,alignItems:"center"}}>
                <span>{AREA_ICON[area]}</span>
                <span style={{fontSize:12,fontWeight:active===area?700:500,color:active===area?AREA_COLOR[area]:"#555",lineHeight:1.3}}>{area}</span>
              </div>
              <div style={{display:"flex",gap:4,marginTop:5,flexWrap:"wrap"}}>
                {has&&<span style={{fontSize:9,background:AREA_COLOR[area]+"20",color:AREA_COLOR[area],padding:"1px 6px",borderRadius:10,fontWeight:700}}>✓ com conteúdo</span>}
                {marks>0&&<span style={{fontSize:9,background:"#fef3c7",color:"#b45309",padding:"1px 6px",borderRadius:10,fontWeight:700}}>🔖 {marks}</span>}
              </div>
            </div>
          );
        })}
        <div style={{marginTop:"auto",padding:"12px 6px 0",borderTop:"1px solid #f0f0f8"}}>
          <div style={{fontSize:10,color:saved?"#10b981":"#bbb",transition:"color .3s"}}>{saved?"✓ Salvo":"● Auto-salvamento"}</div>
        </div>
      </div>
      {/* Editor */}
      <div style={{flex:1,padding:"18px 24px",display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:20}}>{AREA_ICON[active]}</span>
              <h2 style={{fontSize:18,fontWeight:700,fontFamily:"'DM Serif Display',serif",color:AREA_COLOR[active]}}>{active}</h2>
            </div>
            <p style={{fontSize:11,color:"#aaa",marginTop:2}}>Cole imagens com Ctrl+V · Selecione texto para grifar · 🔖 para marcar revisão</p>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {countMarks(notebooks[active])>0&&<span style={{fontSize:11,background:"#fef3c7",color:"#b45309",padding:"5px 10px",borderRadius:20,fontWeight:700}}>🔖 {countMarks(notebooks[active])} marcações</span>}
            <button onClick={exportPDF} style={{fontSize:11,background:"#eef2ff",color:"#6366f1",border:"none",padding:"6px 12px",borderRadius:20,fontWeight:700,cursor:"pointer"}}>📄 Exportar área</button>
            <button onClick={exportAll} style={{fontSize:11,background:"#f0fdf4",color:"#16a34a",border:"none",padding:"6px 12px",borderRadius:20,fontWeight:700,cursor:"pointer"}}>📚 Exportar tudo</button>
          </div>
        </div>
        <NotebookEditor key={active} area={active} content={notebooks[active]||""} onChange={onChange}/>
      </div>
    </div>
  );
}
